...
export function DashboardSidebar() {
	...
	return (
		<div
			className={cn(
				'flex flex-col h-full bg-card dark:bg-[#09090B] border-r border-border relative transition-all duration-300 ease-in-out',
		...
		>
			...
			<div className="flex-1 overflow-y-auto py-6 px-4 space-y-8 custom-scrollbar">
				...
				<div>
					{!isCollapsed ? (
						<p className="text-[11px] font-bold text-slate-400 dark:text-zinc-500 uppercase tracking-wider mb-3 px-2 animate-in fade-in">
							Main Menu
						</p>
					) : (
								...
								<Tooltip key={item.title}>
											...
											<div className="flex items-center gap-3">
												<item.icon
													className={cn(
														...
														isActive
															? 'text-primary'
															: 'text-slate-400 dark:text-zinc-500 group-hover:text-foreground',
													)}
												/>
												{!isCollapsed && (
									...
									{isCollapsed && (
										<TooltipContent
											side="right"
											sideOffset={15}
											className="font-semibold text-xs rounded-lg shadow-md border-border"
										>
											{item.title}
										</TooltipContent>
									)}
			...
			<div className="p-4 border-t border-border mt-auto shrink-0">
				<DropdownMenu>
					...
					>
						...
						{!isCollapsed && (
							<div className="flex flex-col items-start text-sm animate-in fade-in duration-300 min-w-0">
								<span className="font-semibold text-foreground truncate max-w-32.5">
									Sarah Jenkins
								</span>
