	...
	return (
		<div className={cn("flex flex-col h-full bg-card dark:bg-[#09090B] border-r border-border relative transition-all duration-300 ease-in-out", isCollapsed ? "w-20" : "w-64")}>
								...
								<Tooltip key={item.title}>
									<TooltipTrigger asChild>
										...
										>
											<div className="flex items-center gap-3">
												<item.icon
													className={cn("w-4 h-4 shrink-0 transition-colors", isActive ? "text-primary" : "text-slate-400 dark:text-zinc-500 group-hover:text-foreground")}
												/>
											...
											{!isCollapsed && item.badge && (
												<span
													className={cn(
														...
														isActive ? "bg-primary/20 text-primary border border-primary/20" : "bg-slate-100 dark:bg-white/5 border border-border text-muted-foreground",
													)}
												>
													{item.badge}
												</span>
			...
			<div className="p-4 border-t border-border mt-auto shrink-0">
				<DropdownMenu>
					<DropdownMenuTrigger
						className={cn("flex items-center w-full p-2 hover:bg-slate-50 dark:hover:bg-white/5 rounded-xl transition-colors focus:outline-none", isCollapsed ? "justify-center" : "gap-3")}
					>
