		...
		<Sheet open={isOpen} onOpenChange={onClose}>
			<SheetContent className="sm:max-w-md! border-l border-border bg-card dark:bg-[#09090B] p-0 flex flex-col shadow-2xl">
				...
				<SheetHeader className="p-6 sm:p-8 border-b border-border bg-linear-to-br from-ai/5 to-transparent relative overflow-hidden shrink-0">
					...
					<SheetTitle className="text-xl sm:text-2xl font-bold tracking-tight text-foreground">Partner Copilot</SheetTitle>
					<SheetDescription className="text-xs sm:text-sm text-muted-foreground font-medium max-w-[90%]">
						Ask plain-english questions to instantly analyze clinic performance, financials, and retention.
					</SheetDescription>
				</SheetHeader>
				...
				<div className="flex-1 overflow-y-auto p-6 sm:p-8 space-y-8 custom-scrollbar bg-slate-50/30 dark:bg-transparent relative">
					...
					{/* 1. PROACTIVE PARTNER ANALYTICS (TOP) */}
					<div className="space-y-3 relative z-10">
						...
						<div className="p-5 rounded-2xl bg-ai/5 border border-ai/20 shadow-sm">
							<div className="flex items-start gap-3">
								<DollarSign className="w-4 h-4 text-ai shrink-0 mt-0.5" />
								<div>
									<p className="text-sm text-foreground leading-relaxed font-medium">
										Based on the last 30 days, your highest growth partner is <strong className="text-ai font-mono">Smile Arts Dental</strong>. However,{" "}
										<strong className="text-foreground">3 clinics</strong> have exceeded 80% of their credit limit.
									</p>
									<Button onClick={() => onActionClick("debt")} className="w-full mt-4 h-9 rounded-xl bg-ai hover:bg-ai/90 text-white font-bold text-xs shadow-md transition-all">
										Show High-Risk Clinics
									</Button>
								</div>
							</div>
						</div>
					...
					<div className="space-y-3 relative z-10">
						...
						<div className="flex flex-col gap-2.5">
							<button
								onClick={() => onActionClick("suspended")}
							...
							>
								<span>&quot;Show me all suspended accounts.&quot;</span>
							...
							<button
								onClick={() => onActionClick("dormant")}
								className="text-left text-xs sm:text-sm font-medium text-foreground hover:text-ai bg-white dark:bg-[#121214] p-4 rounded-2xl transition-all border border-border hover:border-ai/30 shadow-sm flex items-start justify-between group"
							>
								<span>&quot;Find clinics that haven&apos;t sent a case in 30 days.&quot;</span>
								<ArrowRight className="w-4 h-4 text-muted-foreground opacity-0 group-hover:opacity-100 group-hover:translate-x-1 group-hover:text-ai transition-all mt-0.5 shrink-0" />
							</button>
						</div>
					</div>
				...
				<div className="p-4 sm:p-6 border-t border-border bg-background shrink-0">
					...
					<p className="text-[10px] text-center text-muted-foreground mt-3 font-medium">LabOS AI can make mistakes. Always verify financial data.</p>
				</div>
			</SheetContent>
		</Sheet>
	);
