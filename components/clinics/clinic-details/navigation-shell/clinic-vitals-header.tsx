...
export function ClinicVitalsHeader({ clinic }: { clinic: ClinicDetailsUI }) {
				...
				<div className="flex items-start gap-4">
					...
					<div>
						<div className="flex items-center gap-3 mb-1.5">
							<div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center text-primary shadow-sm border border-primary/20">{getClinicIcon(clinic.type)}</div>
							<h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-foreground">{clinic.name}</h1>
							...
							>
								{isSuspended ? "Account Suspended" : clinic.status}
							</span>
						</div>
						...
						<div className="flex flex-wrap items-center gap-4 text-[11px] font-medium text-muted-foreground ml-14">
							...
							<span className="flex items-center gap-1.5 hover:text-foreground cursor-pointer transition-colors">
								<Phone className="w-3 h-3" /> {clinic.phoneNumber}
							</span>
							<span className="flex items-center gap-1.5 hover:text-foreground cursor-pointer transition-colors">
