...
import { ChevronLeft, Edit3, Download, Mail, Phone, Building2, UserCircle, Hospital, GraduationCap, AlertTriangle, Plus } from "lucide-react";
...
export async function ClinicHeaderSection({ clinicId }: Props) {
	...
	return (
		<div className="px-6 lg:px-8 w-full">
			<div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
				...
				<div className="flex items-start gap-4">
					...
					<div>
						<div className="flex items-center gap-3 mb-1.5">
							...
							<h1 className="text-xl sm:text-2xl font-bold tracking-tight text-foreground">{name}</h1>
						...
						{/* Contact Meta */}
						<div className="flex flex-wrap items-center gap-4 text-[11px] font-medium text-muted-foreground sm:ml-14">
				...
				<div className="flex items-center gap-2 w-full md:w-auto">
					...
					<Button
						...
						className={cn(
							"flex-1 md:flex-none rounded-xl h-10 px-6 font-bold shadow-premium transition-all",
							isSuspended
								? "bg-slate-100 dark:bg-white/5 text-slate-400 dark:text-zinc-500 border border-border cursor-not-allowed"
								: "bg-primary text-primary-foreground hover:bg-primary/90",
						)}
					>
						{isSuspended ? (
							<>
								<Plus className="w-4 h-4 mr-2" /> Blocked
							</>
						) : (
			...
			{isSuspended && (
				<div className="mt-6 sm:ml-14 p-3 rounded-xl bg-destructive/10 border border-destructive/20 flex items-start gap-3 animate-in fade-in slide-in-from-top-2">
