...
export function EditClinicClient({ initialData, clinicId, clinicName, currentBalance }: Props) {
	...
	return (
		<div className="flex flex-col h-full animate-in fade-in duration-700 bg-background relative">
			{/* --- THE EDIT HEADER --- */}
			<header className="shrink-0 flex flex-col md:flex-row md:items-center justify-between gap-4 sm:gap-6 mb-6 sm:mb-8 sticky top-0 z-30 bg-background/80 backdrop-blur-xl pt-4 pb-4 border-b border-border px-4 sm:px-6 lg:px-8 max-w-[1600px] mx-auto w-full">
			...
			<div className="flex-1 min-h-0 relative z-10 w-full">
				<div className="flex flex-col xl:flex-row gap-8 h-full max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8">
					{/* LEFT PANE: The Form */}
					<div className="flex-1 h-full overflow-y-auto custom-scrollbar pb-32">
						<FormProvider {...form}>
					...
					<div className="hidden xl:flex w-96 shrink-0 flex-col gap-6 h-full overflow-y-auto custom-scrollbar pb-12 pt-4">
						<ModificationAuditor control={form.control} initialData={initialData} currentBalance={currentBalance} />
					</div>

					{/* Mobile Floating Auditor */}
					<div className="xl:hidden absolute bottom-0 left-0 right-0 z-40 bg-background/95 backdrop-blur-xl border-t border-border p-4 shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
						<ModificationAuditor control={form.control} initialData={initialData} currentBalance={currentBalance} />
					</div>
				</div>
			</div>
