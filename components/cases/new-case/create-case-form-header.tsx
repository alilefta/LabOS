...
interface CreateCaseFormHeaderProps {
	isSavingDraft: boolean;
	isSubmittingCase: boolean;
	onSaveDraft: () => void;
	control: Control<CreateCaseInput>; // Strictly typed for Create
}

export function CreateCaseFormHeader({ control, isSavingDraft, isSubmittingCase, onSaveDraft }: CreateCaseFormHeaderProps) {
	// ── FORM STATE WATCHERS ──────────────────────────────────────────
	...
	// 1. Draft requires ONLY a patient to be valid
	const isSaveDraftEnabled = !!patientId;

	// 2. Submit requires the core logic loop
	...
	return (
		<header className="shrink-0 flex flex-col md:flex-row md:items-center justify-between gap-4 sm:gap-6 mb-6 sm:mb-8 sticky top-0 z-20 bg-background/80 backdrop-blur-xl pt-4 pb-4 border-b border-border px-4 sm:px-8 shadow-sm">
			{/* LEFT: Title & Context */}
			...
			<div className="flex items-center gap-2 sm:gap-3 w-full md:w-auto mt-1 md:mt-0">
				...
				<Button
					...
					className="flex-2 md:flex-none rounded-xl h-10 px-4 sm:px-6 font-bold shadow-premium transition-all bg-primary text-primary-foreground hover:bg-primary/90"
					type="submit"
