...
interface ClinicalProductConfiguratorProps {
	...
	onPricingPlanSelect: (
		id: string,
		plan: CasePricingPlanDetailsUI | null,
	) => void
	selectedCategoryName: string | null
	selectedJawType: JawType
}

export const ClinicalProductConfigurator = memo(
	function ClinicalProductConfigurator({
		categoryId,
		clinicId,
		selectedWorkTypeId,
		selectedProductId,
		...
		const newCreatedWorkTypeId = useClinicalCreationStore(
			(state) => state.newCreatedWorkTypeId,
		)
		const newCreatedProductId = useClinicalCreationStore(
			(state) => state.newCreatedProductId,
		)
		const newCreatedPricingId = useClinicalCreationStore(
			(state) => state.newCreatedPricingId,
		)
		const consumeNewlyCreated = useClinicalCreationStore(
			(state) => state.consumeNewlyCreated,
		)
		...
		const { data: rawPricingPlans = [], isFetching: isLoadingPricing } =
			useQuery({
				queryKey: ['pricingPlans', selectedProductId],
				queryFn: async () => {
					if (!selectedProductId) return []
					const res = await getPricingPlansByProductAction({
		...
		const handlePricingCreate = useCallback(() => {
			...
			if (selectedProductId) {
				openPricingSheet(
					selectedProductId,
					clinicId,
					selectedJawType === 'OTHER' ? 'BULK' : null,
				)
			}
		...
		useEffect(() => {
			if (newCreatedPricingId && pricingPlans.length > 0) {
				const plan =
					pricingPlans.find((p) => p.id === newCreatedPricingId) || null
		...
		return (
			<div className="flex flex-col gap-5 animate-in fade-in duration-500">
				<div className="flex flex-col gap-2">
					<label className="text-[12px] font-bold text-slate-700 dark:text-zinc-300 uppercase tracking-wider">
						1. Work Type
					</label>
					<SelectorDropdown
						mode={mode} // Pass mode
						...
						placeholder={
							!categoryId ? 'Select Category first' : 'Select Work Type...'
						}
						icon={Layers}
						items={workTypes}
						...
						emptyText={
							selectedJawType === 'OTHER'
								? 'No non-arch departments found.'
								: 'No work types found.'
						}
						type="workType"
					/>
				...
				<div className="flex flex-col gap-2">
					...
					<SelectorDropdown
						...
						emptyText={
							selectedJawType === 'OTHER'
								? 'Only BULK pricing allowed for Maxillofacial.'
...
const SelectorDropdown = memo(function SelectorDropdown(props: SelectorProps) {
	...
	const isLegacyItem =
		!selectedItem && !!value && !!fallbackName && mode === 'edit'
			...
			<PopoverTrigger asChild>
				<Button
					...
					className={cn(
						...
						isDisabled &&
							'opacity-50 cursor-not-allowed bg-slate-50 dark:bg-white/2',
					)}
				>
						...
						>
							{selectedItem ? (
								<span className="flex items-center gap-2">
									<span className="font-bold text-foreground animate-in fade-in duration-300">
										{selectedItem.name}
									</span>
									{mode === 'edit' && (
								...
								<span className="flex items-center gap-2">
									<span className="font-bold text-foreground opacity-60 line-through decoration-amber-500/50">
										{fallbackName}
									</span>
								...
								<span className="font-mono text-muted-foreground animate-pulse text-[11px]">
									Resolving {value.substring(0, 8)}...
								</span>
							) : (
								placeholder
							)}
				...
				<Command className="dark:bg-[#121214]">
								...
								{Array.from({ length: 3 }).map((_, i) => (
									...
									>
										<div className="flex flex-col gap-1.5 w-full">
											...
											{type === 'pricing' && (
												<div className="h-2.5 w-1/3 bg-slate-100 dark:bg-white/5 rounded-md animate-pulse" />
											)}
										</div>
									</div>
								))}
											...
											<div className="flex flex-col min-w-0">
												...
												{type === 'pricing' && 'pricingStrategy' in item && (
													<span className="text-[10px] text-muted-foreground uppercase tracking-wider mt-0.5 flex items-center gap-1.5">
														...
														{formatPricing(item as CasePricingPlanDetailsUI)}
													</span>
												)}
											</div>
											{value === item.id && (
					...
					<div className="p-2 border-t border-border bg-slate-50/80 dark:bg-white/2">
						...
						>
							...
							{mode === 'edit' ? 'Override Selection' : createLabel}
						</Button>
					</div>
				</Command>
			</PopoverContent>
