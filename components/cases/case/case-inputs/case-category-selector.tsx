...
export const CaseCategorySelector = memo(function CaseCategorySelector({
	onCreateNew,
	newCreatedCategory,
	onSelect,
	mode,
	...
	const hasConfiguredItems = currentWorkItems.some(
		(item) => item && item.productId && item.productId.trim() !== '',
	)
	...
	useEffect(() => {
		if (newCreatedCategory?.id) {
			setValue('caseCategoryId', newCreatedCategory.id, {
				shouldValidate: true,
			})
			...
			<div className="flex items-center gap-2">
				...
				<Tooltip delayDuration={300}>
					...
					<TooltipContent className="max-w-50">
						Categories filter the available materials and pricing logic for this
						case.
					</TooltipContent>
				</Tooltip>
			</div>
				...
				{!isLoading && (
					...
					>
						...
						<p className="text-[11px] sm:text-xs text-muted-foreground leading-snug">
							Create a custom clinical category
						</p>
					</button>
				)}
			</div>
				...
				<AlertDialogContent className="rounded-2xl border-border shadow-premium dark:bg-[#121214]">
					<AlertDialogHeader>
						...
						<AlertDialogTitle className="text-xl font-bold tracking-tight">
							{isEdit ? 'Reset Case Work Items?' : 'Change Category?'}
						</AlertDialogTitle>
					...
					<AlertDialogFooter className="mt-6 gap-3 sm:gap-0">
						<AlertDialogCancel className="rounded-xl h-10 font-semibold border-border hover:bg-secondary">
							Keep Current
						</AlertDialogCancel>
						<AlertDialogAction
