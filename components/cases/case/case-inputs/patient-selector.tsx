...
import { Check, ChevronsUpDown, UserPlus, History, MapPin, Phone, CommandIcon } from "lucide-react";
...
export function PatientSelector({ onSelect, onCreateNew, createdPatient }: { onSelect: (id: string) => void; onCreateNew: () => void; createdPatient: PatientDetails | null }) {
	const [open, setOpen] = useState(false);
	const [selectedId, setSelectedId] = useState("");

	const queryClient = useQueryClient();

	const queryKey = ["case", "patients"];
	...
	useEffect(() => {
		...
		queryClient.setQueryData<DataShape>(queryKey, (data): DataShape => {
			...
			const isPatientExists = data.find((patient) => patient.id === createdPatient.id);
			if (!isPatientExists) {
				return [...data, createdPatient];
			}
			return data;
		});
	}, [createdPatient, queryClient]);
			...
			<Popover open={open} onOpenChange={setOpen}>
				...
				<PopoverContent className=" p-0 rounded-2xl border-border shadow-premium overflow-hidden">
					<Command className="dark:bg-[#121214]">
						<CommandInput placeholder="Type patient name..." className="py-2" />
