...
export const KanbanWrapper = memo(function KanbanWrapper({
	serverData,
	onStatusChangeAction,
}: KanbanWrapperProps) {
	// 1. Connect to our high-performance store
	const syncCases = useKanbanStore((state) => state.syncCases)
	const moveCaseOptimistically = useKanbanStore(
		(state) => state.moveCaseOptimistically,
	)
		...
		<div className="h-full w-full flex flex-col relative animate-in fade-in slide-in-from-right-4 duration-500">
			...
			<KanbanInterceptorDialog
				...
				onConfirm={() => {
					if (pendingUpdate) {
						executeTransition(
							pendingUpdate.case.id,
							pendingUpdate.newStatus,
							pendingUpdate.oldStatus,
						)
					}
				}}
			/>
