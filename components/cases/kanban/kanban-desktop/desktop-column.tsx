...
export const DesktopKanbanColumn = memo(function DesktopKanbanColumn({
	id,
	title,
	cases,
}: ColumnProps) {
	...
	return (
		<div className="flex flex-col h-full select-none transition-all duration-300  min-h-150">
			{/* --- COLUMN HEADER --- */}
			...
			<div
				...
				className={cn(
					...
					isOver
						? 'bg-primary/5 border-dashed border-primary/20 shadow-inner'
			...
			>
				...
				<div
					ref={parentRef}
					className="flex-1 overflow-y-auto custom-scrollbar px-1 relative transform-gpu"
				>
					{cases.length > 0 ? (
						<SortableContext
							items={caseIds}
