...
export function DesktopKanbanBoard({ requestStatusTransition }: Props) {
	...
	const handleDragEnd = useCallback(
		(event: DragEndEvent) => {
			...
			if (!VALID_TRANSITIONS[oldStatus]?.includes(newStatus)) {
				toast.error(
					`Invalid move. You cannot transition a case directly from ${oldStatus} to ${newStatus}.`,
				)
				return
			}
	...
	return (
		...
		>
			<div className="h-full  min-h-150 w-full overflow-x-auto pb-6 custom-scrollbar">
			...
			>
				{activeCase ? (
					<DesktopKanbanCard caseItem={activeCase} isOverlay />
				) : null}
			</DragOverlay>
		</DndContext>
	)
