...
import { Filter, X, Activity, Check, CalendarRange, Box } from "lucide-react";
...
export const AdvancedFiltersSheet = memo(function AdvancedFiltersSheet({ isOpen, onClose, currentFilters, onApplyFilters, onClearFilters, mode }: Props) {
	...
	// ── 2. DYNAMIC VISIBILITY GUARDS (CONTEXT AWARE) ──────────────────────────
	// - Hide clinic if we are already in the Clinic's historical ledger tab
	const showClinicSelector = mode !== "CLINIC_HISTORY";

	// - Hide staff selector if we are in the Technician's active OR historical tab
	const showStaffSelector = mode !== "STAFF_DASHBOARD" && mode !== "STAFF_HISTORICAL";

	// - Show the quality/remake card in both Clinic & Staff historical views
	const showRemakeToggle = mode === "CLINIC_HISTORY" || mode === "STAFF_HISTORICAL";

	// ── 3. DYNAMIC STATUS OPTIONS ─────────────────────────────────────────────
	const visibleStatuses = useMemo(() => {
		// Clinical History & Staff History both restrict selections to terminal outcomes [3]
		if (mode === "CLINIC_HISTORY" || mode === "STAFF_HISTORICAL") {
			return (["COMPLETED", "DELIVERED", "FAILED"] as CaseStatus[]).map((id) => ({ id, ...STATUS_REGISTRY[id] }));
		}
		// Active Technician workbench strictly shows WIP stages
		if (mode === "STAFF_DASHBOARD") {
			return (["ASSIGNED", "PROCESSING"] as CaseStatus[]).map((id) => ({ id, ...STATUS_REGISTRY[id] }));
		}
		// Global directory shows everything
	...
	return (
		<Sheet open={isOpen} onOpenChange={handleOpenChange}>
			<SheetContent showCloseButton={false} className="sm:max-w-md! border-l border-border bg-card dark:bg-[#09090B] p-0 flex flex-col shadow-2xl">
				...
				{/* --- BODY --- */}
				<div className="flex-1 overflow-y-auto p-6 sm:p-8 flex flex-col gap-10 custom-scrollbar">
					{/* 1. STATUS FILTER */}
					<div className="flex flex-col gap-2.5">
						<h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/60 flex items-center gap-2 mb-1">
							<Activity className="w-4 h-4 text-primary/70" /> {mode === "CLINIC_HISTORY" || mode === "STAFF_HISTORICAL" ? "Case Outcomes" : "Workflow Stage"}
						</h3>
					...
					{/* Quality/Remake Toggle (Visible in both history modes) */}
					{showRemakeToggle && (
						<div className="flex items-center justify-between p-4 rounded-xl border border-amber-500/20 bg-amber-500/5 animate-in fade-in zoom-in-95">
							...
							<Switch
								checked={localFilters.isRemakeOnly}
								onCheckedChange={(v) => setLocalFilters((prev) => ({ ...prev, isRemakeOnly: v }))}
							...
							{(["createdAt", "deadline", "deliveredAt"] as CaseDateFilterField[])
								...
								.map((field) => {
									...
									return (
										...
										>
											{field === "createdAt" ? "Registration" : field === "deadline" ? "Deadline" : "Delivery"}
										</button>
									);
								})}
						</div>
					...
					<div className="flex flex-col gap-6 pt-8 border-t border-border/50">
						<h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/60 flex items-center gap-2 mb-1">
							<Box className="w-4 h-4 text-primary/70" /> Urgency & Logistics
						</h3>

						{/* Rush Toggle (Standard across modes) */}
						<div className="grid grid-cols-1 gap-3">
							{/* --- DYNAMIC TOGGLE 1: OVERDUE (Critical - Rose) --- */}
							{mode !== "CLINIC_HISTORY" && mode !== "STAFF_HISTORICAL" && (
								<div
									className={cn(
										"flex items-center justify-between p-4 rounded-2xl border transition-all duration-300 shadow-sm",
										localFilters.isOverdueOnly
								...
								>
									<div className="flex flex-col gap-1 pr-4">
										<span className={cn("text-[13px] font-bold transition-colors", localFilters.isOverdueOnly ? "text-rose-700 dark:text-rose-500" : "text-foreground")}>
											Overdue Cases Only
										</span>
										<span className="text-[10px] text-muted-foreground leading-tight">Isolate cases that have passed their committed deadline.</span>
									</div>
									<Switch
										checked={localFilters.isOverdueOnly}
										onCheckedChange={(v) => setLocalFilters((prev) => ({ ...prev, isOverdueOnly: v }))}
										className="data-[state=checked]:bg-rose-500 shadow-sm shrink-0"
									/>
								</div>
							)}

							{/* --- DYNAMIC TOGGLE 2: RUSH (Warning - Amber) --- */}
							{/* Rush Switch (Only relevant for active workloads) */}
							{mode !== "CLINIC_HISTORY" && mode !== "STAFF_HISTORICAL" && (
								<div
									className={cn(
										"flex items-center justify-between p-4 rounded-2xl border transition-all duration-300 shadow-sm",
								...
								>
									...
									<Switch
										...
										className="data-[state=checked]:bg-amber-500 shadow-sm shrink-0"
									/>
								</div>
							)}

							{showClinicSelector && (
								<ClinicFilterSelector value={localFilters.clinicId} onSelect={(id) => setLocalFilters((prev) => ({ ...prev, clinicId: id }))} label={"Origin Clinic"} />
							)}
							...
							{showStaffSelector && (
								<StaffFilterSelector
									...
									variant={mode === "CLINIC_HISTORY" ? "emerald" : "default"}
								/>
							)}
						</div>
					</div>
