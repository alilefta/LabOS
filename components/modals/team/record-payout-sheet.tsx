...
import { Banknote, Smartphone, CreditCard, Building, Loader2, CalendarIcon, Coins, CheckCircle2, LucideIcon } from "lucide-react";
...
// Schemas & Actions
import { RecordPayoutInput, RecordPayoutInputSchema } from "@/schema/composed/team/record-payout.schema";
...
const PAYMENT_METHODS: { id: PaymentMethod; label: string; icon: LucideIcon; colorClass: string }[] = [
	{ id: "CASH", label: "Cash Payout", icon: Banknote, colorClass: "text-emerald-500 group-hover:text-emerald-500" },
	...
	{ id: "OTHER", label: "Other", icon: Coins, colorClass: "text-muted-foreground group-hover:text-foreground" },
];
...
export function RecordPayoutSheet({ isOpen, onClose, staffId, staffName, pendingCases }: Props) {
	...
	const { executeAsync: issuePayout, isExecuting } = useAction(issueStaffPayoutAction, {
		onSuccess: () => {
			toast.success(`Commissions paid to ${staffName.split(" ")[0]}!`);

			// INSTANT CACHE SYNC: Invalidate the payroll queries [3]
			queryClient.invalidateQueries({ queryKey: ["staff-payroll-vitals", staffId] });
			queryClient.invalidateQueries({ queryKey: ["pending-commissions", staffId] });
		...
		<Sheet open={isOpen} onOpenChange={onClose}>
			<SheetContent className="sm:max-w-md! border-l border-border bg-card dark:bg-[#09090B] p-0 flex flex-col shadow-2xl">
				...
				<SheetHeader className="p-8 border-b border-border bg-linear-to-br from-emerald-500/5 to-transparent relative overflow-hidden shrink-0">
					...
					<SheetTitle className="text-2xl font-bold tracking-tight text-foreground">Issue commission payout</SheetTitle>
					<SheetDescription className="text-sm text-muted-foreground font-medium">
						Generate payout receipt and clear the unbilled queue for <strong className="text-foreground">{staffName}</strong>.
					</SheetDescription>
				</SheetHeader>
				...
				<div className="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar">
					...
					<div className="flex items-center justify-between p-4 rounded-xl border border-border bg-slate-50 dark:bg-white/2">
						...
						<div className="flex flex-col items-end">
							<span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Payout Value ({pendingCases.length} Cases)</span>
							<span className="text-lg font-mono font-bold text-emerald-600 dark:text-emerald-400">{formatMoney(totalAmountDue)}</span>
						</div>
					</div>

					<form id="record-payout-form" onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
						{/* 1. PAYMENT METHOD (Visual Grid) */}
						<div className="space-y-3">
							<label className="text-[13px] font-bold text-slate-700 dark:text-zinc-300">Payout Method</label>
						...
						<div className="grid grid-cols-1 sm:grid-cols-2 gap-5 pt-2">
							<Controller
								...
								render={({ field, fieldState }) => (
									<CustomFieldWithLabel field={field} fieldState={fieldState} fieldTitle="Payout Date" nameInSchema="paidAt">
										<Popover>
							...
							<Controller
								...
								name="reference"
								render={({ field, fieldState }) => (
									<InputWithLabel field={field} fieldState={fieldState} fieldTitle="Txn / Ref Number" nameInSchema="reference" placeholder="e.g. TXN-99841" isOptional />
								)}
							/>
						</div>
				...
				<SheetFooter className="p-8 border-t border-border bg-slate-50/30 dark:bg-white/1 shrink-0">
					...
					>
						{isExecuting ? <Loader2 className="animate-spin w-4 h-4" /> : <>Disburse Payout</>}
					</Button>
				</SheetFooter>
			</SheetContent>
		</Sheet>
