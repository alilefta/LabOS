...
export function RegisterStaffMemberSheet({
	...
	const selectedCommission = useWatch({
	...
	const handleWhatsAppInvite = (token: string) => {
		...
		window.open(
			`https://wa.me/${form.getValues('phoneNumber').replace(/\D/g, '')}?text=${msg}`,
			'_blank',
		)
	}
							...
							{successPayload.token ? (
								<div className="flex flex-col gap-4 animate-in fade-in duration-300">
									<p className="text-sm text-muted-foreground text-center">
										Software invitation generated. Send this secure onboarding
										link directly to the employee.
									</p>
									<div className="flex items-center gap-2 p-1.5 bg-slate-50 dark:bg-white/5 border border-border rounded-xl">
										...
										<Button
											...
											className={cn(
												'rounded-lg h-9 font-bold text-xs px-3',
												copied
									...
									>
										<MessageSquare className="w-4 h-4" /> Share Onboarding Link
										via WhatsApp
									</Button>
								</div>
							) : (
								<p className="text-sm text-muted-foreground text-center leading-relaxed">
									Profile successfully recorded. Since no software access was
									selected, this technician can now be assigned to cases on work
									tickets, but cannot log into the system.
								</p>
							)}
						</div>
					...
					<>
						<SheetHeader className="p-8 border-b border-border bg-linear-to-br from-primary/5 to-transparent relative overflow-hidden shrink-0">
							...
							<SheetDescription className="text-sm text-muted-foreground font-medium">
								Onboard a new employee and define their role, commission, and
								system access.
							</SheetDescription>
						</SheetHeader>
						...
						<div className="flex-1 overflow-y-auto p-8 flex flex-col gap-8 custom-scrollbar">
							<FormProvider {...form}>
								...
								>
									...
									{/* SECTION 4: SYSTEM ACCESS & ROLE (SLIDE DOWN ACCORDION) */}
										...
										<div className="flex items-center justify-between p-4 rounded-xl bg-slate-50 dark:bg-white/5 border border-border shadow-sm">
											<div className="flex flex-col gap-0.5 pr-4">
												...
												<span className="text-[10px] text-muted-foreground leading-snug">
													Allow this worker to log into the software and manage
													their desk.
												</span>
											</div>
											<Controller
												...
												<Controller
													...
													render={({ field, fieldState }) => (
														<InputWithLabel
															field={field}
															fieldState={fieldState}
															fieldTitle="System Invitation Email"
															nameInSchema="email"
													...
													<Controller
														...
														render={({ field }) => (
															<div className="flex p-1 bg-slate-100 dark:bg-white/5 rounded-xl border border-border h-11">
																{(
																	['STAFF', 'ADMIN', 'MANAGER'] as LabRole[]
																).map((role) => (
																	<button
																		key={role}
																		type="button"
																		...
																		className={cn(
																			...
																			field.value === role
																				? 'bg-white dark:bg-[#121214] text-primary shadow-sm'
									...
									<div className="flex flex-col gap-5">
										...
										<p className="text-[10px] text-muted-foreground font-medium px-1">
											The System Role (above) controls permissions, but the
											Display Title is what shows on invoices and the clinic
											portal.
										</p>
									</div>
