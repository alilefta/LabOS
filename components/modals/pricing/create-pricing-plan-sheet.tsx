...
export const CreatePricingPlanSheet = memo(function CreatePricingPlanSheet() {
	const isPricingSheetOpen = useClinicalCreationStore(
		(state) => state.isPricingSheetOpen,
	)
	const closeAllSheets = useClinicalCreationStore(
		(state) => state.closeAllSheets,
	)
	const activeProductId = useClinicalCreationStore(
		(state) => state.activeProductId,
	)
	const activeClinicId = useClinicalCreationStore(
		(state) => state.activeClinicId as string | null,
	)
	...
	const requiredPricingStrategy = useClinicalCreationStore(
		(state) => state.requiredPricingStrategy,
	)
	...
	const { executeAsync: createPlan, isExecuting } = useAction(
		createPricingPlanAction,
		{
			onSuccess: ({ data }) => {
				...
				queryClient.invalidateQueries({
					queryKey: ['pricingPlans', activeProductId],
				})
		...
		const all = [
			{
				...
				sub: 'Every selected tooth costs exactly the same amount.',
			},
			{
				id: 'BULK',
				label: 'Flat Rate / Arch',
				sub: 'A single flat fee regardless of the number of teeth mapped.',
			},
			{
				id: 'CUSTOM',
				label: 'Tiered & Hybrid',
				sub: 'Different price for the first unit, with optional bulk capping.',
			},
		]
	...
	const onSubmit = async (data: CreateCaseItemPricingPlanInput) => {
		...
		const payload = {
			...data,
			clinicId:
				pricingScope === 'CUSTOM_CLINIC' && activeClinicId
					? activeClinicId
					: undefined,
			...
			<SheetContent className="sm:max-w-md! border-l border-border bg-card dark:bg-[#09090B] p-0 flex flex-col shadow-2xl">
				...
				<div className="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar">
					<WorkTypeBlueprintHierarchy
						...
						isCreatingPriceOnly={true}
					/>

					<FormProvider {...form}>
						<form
							id="create-pricing-form"
							...
							<Controller
								...
								render={({ field, fieldState }) => (
									<InputWithLabel
										...
										nameInSchema="name"
										placeholder="e.g. Standard Zirconia Rate"
									/>
								)}
							/>
							...
							<div className="space-y-3">
								...
								<div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
									...
									>
										...
										<span className="text-[10px] text-muted-foreground leading-snug">
											{!activeClinicId
												? 'Only available when creating a case for a specific clinic.'
												: 'Locked exclusively to the current active clinic.'}
										</span>
									</button>
								</div>
							</div>
								...
								<div className="grid grid-cols-1 gap-3">
									{visibleStrategies.map((strategy) => (
										<button
											...
											type="button"
											onClick={() =>
												handleStrategyChange(strategy.id as PricingStrategy)
											}
											className={cn(
								...
								{selectedStrategy === 'BULK' && (
									<div className="space-y-2 animate-in fade-in slide-in-from-top-2 duration-300">
										...
										<p className="text-[10px] font-medium text-muted-foreground flex items-start gap-1.5 ml-1 mt-1.5">
											...
											This single price will be billed regardless of how many
											teeth the clinician maps.
										</p>
									</div>
								)}
										...
										<div className="pt-2">
											<div className="flex items-center justify-between p-4 rounded-xl bg-white dark:bg-white/2 border border-border shadow-sm">
												<div className="flex flex-col gap-0.5 pr-4">
													...
													<span className="text-[10px] text-muted-foreground leading-snug">
														If the case exceeds a certain number of units, cap
														the price at a flat fee.
													</span>
												</div>
												<Switch
