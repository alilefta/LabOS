...
export const StaffRosterGrid = memo(function StaffRosterGrid({ staff, canViewFinancials, onEdit, onToggleStatus, onInvite, onCreateNew }: StaffRosterGridProps) {
	...
	return (
		<div className="flex flex-col gap-6 animate-in fade-in duration-500 w-full min-h-0">
			...
			{isEmpty && (
				<div className="w-full h-80 rounded-4xl border-2 border-dashed border-border flex flex-col items-center justify-center p-8 text-center animate-in fade-in zoom-in-95 duration-500 relative bg-slate-50/30 dark:bg-white/[0.01]">
					...
					<div className="relative z-10 max-w-sm flex flex-col items-center gap-4">
						...
						<div className="space-y-1.5">
							...
							<p className="text-xs text-muted-foreground leading-relaxed">No active employees match your current search queries or role filters.</p>
						</div>
						<Button
							type="button"
							onClick={onCreateNew}
							className="h-9 rounded-xl bg-primary text-primary-foreground hover:bg-primary/90 font-bold text-xs px-5 shadow-sm mt-2 transition-all"
						>
							<UserPlus className="w-3.5 h-3.5 mr-1.5" /> Onboard New Member
						</Button>
					</div>
				</div>
			)}
