...
export function StaffCompensationVitals({ vitals }: Props) {
	...
	// UX Logic: Get the current month abbreviation for the YTD Badge
		...
		<div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-in slide-in-from-bottom-2 duration-500">
			...
			<div className="lab-card p-6 flex flex-col justify-between relative overflow-hidden group min-h-36">
				<div className="flex items-center justify-between mb-4 relative z-10 w-full">
					...
					<span className="px-2 py-0.5 rounded-md bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-[9px] font-bold uppercase tracking-widest border border-emerald-500/20 shadow-sm">
						{isPercentage ? 'Percentage' : 'Fixed Rate'}
					</span>
				</div>

				<div className="mt-auto relative z-10 min-w-0">
					<p className="text-2xl sm:text-3xl font-mono font-black text-foreground tracking-tighter truncate">
						{isPercentage
							? `${vitals.commissionValue}%`
							: formatMoney(vitals.commissionValue)}
					</p>
					<p className="text-[10px] text-muted-foreground font-medium mt-1">
						{isPercentage
							? 'Of grand total per completed case.'
							: 'Flat fee paid per completed case unit.'}
					</p>
				</div>
			</div>
			...
			>
				...
				<div className="mt-auto relative z-10 min-w-0">
					<p
						className={cn(
							'text-2xl sm:text-3xl font-mono font-black tracking-tighter transition-colors truncate',
							hasPending
					...
					<p className="text-[10px] text-muted-foreground font-medium mt-1">
						{hasPending
							? 'Earned commissions awaiting statement run.'
							: 'All completed cases fully settled.'}
					</p>
				</div>
			</div>
			...
			<div className="lab-card p-6 flex flex-col justify-between relative overflow-hidden group min-h-36">
				<div className="flex items-center justify-between mb-4 relative z-10 w-full">
					...
					<span className="px-2 py-0.5 rounded-md bg-slate-100 dark:bg-white/5 text-muted-foreground border border-border text-[9px] font-bold uppercase tracking-widest">
						Jan - {currentMonth}
					</span>
				</div>
				...
				<div className="mt-auto relative z-10 min-w-0">
					<p className="text-2xl sm:text-3xl font-mono font-black text-foreground tracking-tighter truncate">
						{formatMoney(vitals.totalYtdEarnings)}
					</p>
					<p className="text-[10px] text-muted-foreground font-medium mt-1">
						Total net payouts logged in the current calendar year.
					</p>
				</div>
			</div>
		</div>
