...
import { Coins, Loader2, Save, Percent, DollarSign, Sparkles } from "lucide-react";
...
import { UpdateStaffCompensationInput, UpdateStaffCompensationInputSchema } from "@/schema/composed/team/staff-settings.schema";
...
const COMMISSION_OPTIONS = [
	{ id: "PERCENTAGE", label: "Percentage Share", icon: Percent, desc: "Earn a cut of case revenue" },
	{ id: "FIXED", label: "Fixed Fee", icon: DollarSign, desc: "Earn a flat rate per case" },
];

export function StaffCompensationCard({ initialData }: Props) {
	...
	const selectedType = useWatch({ control: form.control, name: "commissionType" });
	const commissionVal = useWatch({ control: form.control, name: "commissionValue" }) || 0;

	// --- SERVER ACTION ---
	const { executeAsync: updateCompensation, isExecuting } = useAction(updateStaffCompensationAction, {
		onSuccess: () => {
		...
		onError: ({ error }) => {
			if (error.serverError) {
				toast.error("Access Denied", { description: error.serverError.message });
			} else {
	...
	return (
		<div className="lab-card overflow-hidden flex flex-col relative group transition-all duration-300 border-emerald-500/10 bg-emerald-500/[0.01]">
			...
			<div className="p-6 border-b border-border bg-slate-50/50 dark:bg-white/[0.02] flex items-center gap-3 relative z-10">
				...
				<div>
					<h3 className="text-sm font-bold text-foreground">Compensation & Commissions</h3>
					<p className="text-[10px] text-muted-foreground uppercase tracking-widest font-medium">Default Payroll Settings</p>
				</div>
			</div>

			{/* --- CARD BODY --- */}
			<div className="p-6 sm:p-8 space-y-6 relative z-10">
				<form id={`compensation-form-${initialData.staffId}`} onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
					{/* Commission Type Segmented Toggle */}
					<div className="space-y-3">
						<label className="text-[13px] font-semibold text-slate-700 dark:text-zinc-300">Commission model</label>
						<Controller
										...
										return (
											<button
												...
												onClick={() => {
													...
													form.setValue("commissionValue", 0); // Reset value to avoid schema type clashes
												}}
												className={cn(
													...
													isSelected
														...
														: "bg-card border-border hover:border-slate-300 dark:hover:border-white/10",
												)}
											>
												<div
													className={cn(
												...
												<div className="flex flex-col">
													<span className={cn("text-xs font-bold leading-tight mb-0.5", isSelected ? "text-foreground" : "text-muted-foreground")}>{opt.label}</span>
													<span className="text-[10px] text-muted-foreground font-medium">{opt.desc}</span>
												</div>
											</button>
										);
									})}
					...
					<Controller
						...
						render={({ field, fieldState }) => (
							<InputWithLabel
								...
								fieldTitle={selectedType === "PERCENTAGE" ? "Commission Percentage (%)" : "Flat Fee Per Unit ($)"}
					...
					<div className="p-4 rounded-2xl bg-emerald-500/5 border border-emerald-500/20 flex gap-3 items-start animate-in fade-in slide-in-from-top-2 duration-300">
						...
						<p className="text-[11px] text-muted-foreground leading-relaxed font-medium">
							<span className="text-emerald-500 font-bold uppercase">Payroll Preview:</span>{" "}
							{selectedType === "PERCENTAGE" ? (
								<>
									This employee will earn <strong className="text-foreground">{commissionVal}%</strong> of the grand total of any cases completed on their bench.
								</>
							) : (
								<>
									This employee will earn a flat fee of <strong className="text-foreground">${Number(commissionVal).toFixed(2)}</strong> for every case completed on their bench.
								</>
							)}
						</p>
					</div>
