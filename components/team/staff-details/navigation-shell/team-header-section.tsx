...
import { ChevronLeft, Edit3, MoreVertical, Mail, Phone, ShieldAlert } from "lucide-react";
		...
		<div className="px-6 lg:px-8 w-full max-w-500 mx-auto">
			<div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
				...
				<div className="flex items-start gap-4">
					...
					<div className="flex items-start gap-4">
						...
						<Avatar className={cn("w-12 h-12 border-2 border-background shadow-md ring-1 ring-border", isInactive ? "opacity-50 grayscale" : "ring-primary/20")}>
							<AvatarImage src={staff.avatarUrl || `https://api.dicebear.com/7.x/notionists/svg?seed=${staff.id}`} />
						...
						<div className="flex flex-col min-w-0">
							<div className="flex items-center gap-3">
								...
								>
									{isInactive ? "Deactivated" : staff.accessState === "ACTIVE_USER" ? "Active User" : staff.accessState === "PENDING_INVITE" ? "Pending" : "No Access"}
								</span>
							</div>
							...
							<p className="text-xs font-bold text-primary/80 uppercase tracking-wider mt-1">
								...
								{staff.specialization && <span className="text-muted-foreground font-medium lowercase"> • {staff.specialization}</span>}
							</p>

							{/* Contact Details */}
							<div className="flex flex-wrap items-center gap-4 text-[11px] font-medium text-muted-foreground mt-3">
								<span className="flex items-center gap-1.5">
									<Phone className="w-3.5 h-3.5" /> <span className="font-mono">{staff.phoneNumber}</span>
								</span>
								{staff.inviteEmail && (
									<span className="flex items-center gap-1.5">
			...
			{isInactive && (
				<div className="mt-6 ml-16 p-3 rounded-xl bg-destructive/10 border border-destructive/20 flex items-start gap-3 animate-in fade-in slide-in-from-top-2 max-w-xl">
					...
					<div>
						<h4 className="text-[12px] font-bold text-destructive">Account Deactivated</h4>
						<p className="text-[11px] text-destructive/80 font-medium leading-snug mt-0.5">
							This employee was manually deactivated. They can no longer log in or be assigned to active cases. Historical logs and commissions remain preserved.
						</p>
					</div>
				</div>
			)}
