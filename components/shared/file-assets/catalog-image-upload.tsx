 ...
 * @returns
 */
export function CatalogImageUpload<S extends FieldValues>({
	nameInSchema,
	label,
}: Props<S>) {
	...
	const errorMessage = formState.errors?.[nameInSchema as string]
		?.message as string
	return (
			...
			<div
				...
				className={cn(
					...
					isDragActive
						...
						: 'border-border bg-slate-50 dark:bg-white/2 hover:border-primary/50 hover:bg-slate-100 dark:hover:bg-white/5',
					// Preview state
				...
				{preview ? (
					...
					<div className="flex flex-col items-center gap-2 text-muted-foreground group-hover:text-foreground transition-colors">
						<div className="p-2.5 bg-white dark:bg-[#121214] rounded-xl shadow-sm ring-1 ring-border group-hover:ring-primary/50 transition-all">
							<ImagePlus
								size={20}
								className="text-slate-400 dark:text-zinc-500 group-hover:text-primary transition-colors"
							/>
						</div>
