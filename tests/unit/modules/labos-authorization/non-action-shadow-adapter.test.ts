...
import { evaluateN001TeamDirectoryAuthorizationShadow } from '@/modules/labos-authorization/non-action-shadow-adapter'
...
function options(events: LabOSShadowMonitorEvent[]) {
	return {
...
describe('N-001 Team directory shadow adapter', () => {
	it.each(['owner', 'admin'])(
		...
		async (role) => {
			...
			const result = await evaluateN001TeamDirectoryAuthorizationShadow(
				{ tenant: tenant(role) },
				options(events),
			)
	...
	it('counts unknown roles without exposing raw role values', async () => {
		const events: LabOSShadowMonitorEvent[] = []
		await evaluateN001TeamDirectoryAuthorizationShadow(
