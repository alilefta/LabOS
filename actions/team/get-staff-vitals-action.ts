	...
	.action(async ({ ctx }) => {
		...
		try {
			...
			const [
				...
				currentCompletedCases,
				previousCompletedCases,
			] = await Promise.all([
			...
			const labCapacityPct =
				totalAvailableCapacity > 0
					? Math.min(
							Math.round(
								(activeAssignmentsCount / totalAvailableCapacity) * 100,
							),
							100,
						)
					: 0
			...
			const calculateAverageDays = (
				casesList: typeof currentCompletedCases,
			) => {
				...
				const totalHours = casesList.reduce((sum, c) => {
					...
					return (
						sum +
						differenceInHours(new Date(c.completedAt), new Date(c.createdAt))
					)
				}, 0)
			...
			if (avgPrevious > 0) {
				turnaroundDeltaPercent =
					Math.round(((avgCurrent - avgPrevious) / avgPrevious) * 100 * 10) / 10
