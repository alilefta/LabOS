...
export const getClinicPricingPlansAction = actionClientWithLab
	...
	.action(async ({ ctx, parsedInput }) => {
		...
		try {
			...
			function resolveHeadlinePrice(plan: {
				pricingStrategy: PricingStrategy
				bulkPrice: unknown
				toothPrice: unknown
				firstToothPrice: unknown
			}): number | null {
				switch (plan.pricingStrategy) {
					...
					case 'CUSTOM':
						return plan.firstToothPrice !== null
							? Number(plan.firstToothPrice)
							: null
				}
			}
			...
			const plans: ClinicPricingPlanDTO[] = clinicPlans.map((p) => {
				const defaultPlan = p.productId
					? (defaultPlanMap.get(p.productId) ?? null)
					: null

				const clinicPrice = resolveHeadlinePrice(p)
				const standardPrice = defaultPlan
					? resolveHeadlinePrice(defaultPlan)
					: null

				const discountPercent =
					clinicPrice !== null && standardPrice !== null && standardPrice > 0
						? Math.round(
								((standardPrice - clinicPrice) / standardPrice) * 100 * 10,
							) / 10
						: null
				...
				return {
					...
					details: {
						...
						firstToothPrice:
							p.firstToothPrice !== null ? Number(p.firstToothPrice) : null,
						additionalToothPrice:
							p.additionalToothPrice !== null
								? Number(p.additionalToothPrice)
								: null,
						teethCountToApplyBulkPrice:
							p.teethCountToApplyBulkPrice !== null
								? Number(p.teethCountToApplyBulkPrice)
								: null,
					},
					standardComparison:
						standardPrice !== null
							? { price: standardPrice, discountPercent }
							: null,
					clinicDetails: {
						id: p.clinic?.id ?? clinicId,
						name: p.clinic?.name ?? 'N/A',
					},
				}
			})
