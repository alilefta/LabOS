...
The initial runtime pilot is deliberately limited to A-124 and A-125. Only their client declarations change; their handler bodies, Better Auth calls, mutation ordering, response shapes, and domain errors remain intact. A repository architecture test enumerates shadow-client consumers and requires exactly those two paths while explicitly excluding A-083 and A-123. Generic membership operations and every other LabOS action remain on their prior clients. This makes rollback a two-declaration change and prevents incidental expansion of the experiment.
