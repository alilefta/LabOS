...
N-001 enforcement-adapter checkpoint (updated 2026-08-26): a dedicated server-only adapter converts canonical TenantContext identity into the generic actor and compares the current verified-tenant-member page decision with `membership.list`. It reuses the established four comparison categories and sanitized Axiom-compatible event, with `legacyRequiredRole: null` because the current boundary is membership-based rather than hierarchy-based. The deployment-owned mode now selects authority: shadow/rollback preserve the legacy page, while V1 enforces Owner/Admin allow and Manager/Staff/unknown-role denial. V1 evaluation failure denies in V1 mode; legacy evaluation failure is observational after cutover. Identity, correlation, failure, role-matrix, and isolation tests exclude user/member IDs and raw role tokens.
...
N-001 loader checkpoint (updated 2026-08-26): a server-only orchestration loader enforces the required order `requireTenantContext -> N-001 comparison/enforcement -> tenant-scoped Member repository`. A denial or failure from the deployment-selected authority prevents all data loading. Shadow and rollback preserve the verified-member legacy result; V1 denies on missing permission, unknown role, or evaluation failure. Only canonical Organization/Lab IDs are forwarded to the repository, pagination remains bounded there, and only the client-safe directory DTO is returned. Ordering, authoritative denial/failure short-circuit, tenant-resolution failure, and Organization switching are tested.
