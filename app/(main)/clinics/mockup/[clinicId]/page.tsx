	...
	return (
		<ClinicTerminalShell status={clinic.status} balance={clinic.currentBalance} limit={clinic.creditLimit}>
			...
			<div className="sticky top-0 z-30 bg-background/80 backdrop-blur-xl border-b border-border pt-6 flex flex-col gap-6">
				<ClinicHeaderSection name={clinic.name} id={clinic.id} type={clinic.type} email={clinic.email} phoneNumber={clinic.phoneNumber} status={clinic.status} />
					...
					<div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
						...
						<div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
							...
							<div className="lg:col-span-8 flex flex-col">
								<ProductionQualityHeatmap clinicName={clinic.name} heatmapData={[]} />
							</div>
						</div>
						...
						<AiRelationshipAuditor
							...
							meta={{
								...
								currentCases: 30,
								previousCases: 30,
								totalCasesInPeriod: 30,
								labFaultRemakes: 30,
								clinicFaultRemakes: 30,
								invoicesEvaluated: 30,
							}}
							scores={{
								volume: 20,
								logic: 30,
